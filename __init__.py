...
def clear_html_tags2(raw_question: str) -> str:
    return HTML_PATTERN.sub('', raw_question)
...
def get_kanji_from_cards() -> Set[str]:
    ...
    for card_id in cards:
        ...
        kanji_set.update(
            ch for ch in clear_html_tags(
                card.render_output().question_text
            ) if KANJI_PATTERN.match(ch) and card.factor != 0
        )
    return kanji_set
...
def check_joyo() -> None:
    try:
        ...
        report = [
            ...
            f"Total unique kanji taken from deck: {len(deck_kanji)}",
...
def get_loan_words_cards() -> (list[str], int):
    cards = mw.col.find_cards("")
    word_list = list()
    count = 0
    for card_id in cards:
        card = mw.col.get_card(card_id)
        word = clear_html_tags2(card.render_output().question_text)
        if card.factor != 0:
            count += 1
...
def check_loan_words() -> None:
    try:
        katakana_deck = get_loan_words_cards()
        report = [
            f'Total taken cards from deck: {katakana_deck[1]}',
            f'Amount of loan words: {len(katakana_deck[0])}',
            f'Percentage: {((len(katakana_deck[0]) / katakana_deck[1]) * 100):.1f}%',
        ]
        showInfo("\n".join(report))
    except Exception as e:
...
action2 = QAction("Check loan word amount", mw)

qconnect(action2.triggered, check_loan_words)
qconnect(action.triggered, check_joyo)
